...
% Evaluate results when an "add-one-in" approach is taken (Supplemental File)
netfile='./Data/InSilicoDataBG.mat';
CalcBGNets(datafile, netfile);
otag='./Figures/Figure1/VizPatternsBG'; % '' triggers the program to not save the figures
for(bgcnt=1:3)
	VizSSNPatternsBG(netfile, otag, NodePairs, bgcnt);
	close all;
end
...
%{
% for calculating and visualizing LIONESS::WPCC, potential Supplemental Figure 8
otag='Figures/Figure3/PopulationVarLWPCC'; % output tag for saving the figures
vizdatafile='./Data/GTExVarDataLWPCC.mat';
EvalSubpopByVarOverTopEdges_LWPCC(datafile, selTissue, NumSamp, PerVals, PerLabs, NodeIdx, NumRnds, vizdatafile);
VizSubpopByVarOverTopEdges_LWPCC(vizdatafile, otag);
close all;
%}
...
%{
% for non-linear methods / Supplemental Figure 7
% note this takes a *VERY* long time, so it is commented out
NumRGenes=100;
dtag='./Data/MixNLVals';
seltype={'low', 'rand', 'high'};
for(scnt=1:length(seltype))
	EvalMethodsMixtureNL(datafile, NumRGenes, seltype{scnt}, RandIdx, dtag);
end

% visualize the results
otag='./Figures/Figure4/MixEvalNL'; % otag='';
VizMethodsMixtureNL(dtag, seltype, RandIdx, TissueLegend, otag);
close all;
%}
